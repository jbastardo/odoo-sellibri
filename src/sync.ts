...
async function getProductTitle(product: odoo.OdooProduct): Promise<string> {
  // Since we fetch with context: { lang: 'es_VE' }, product.name is always the fully translated name!
  // No need to query product.template for the title anymore.
...
async function buildFullPayload(
  ...
  logger.info(MODULE, `buildFullPayload SKU=${odooProduct.default_code}: qty_available=${odooProduct.qty_available}, virtual_available=${odooProduct.virtual_available}, sending available=${Math.max(0, Math.floor(odooProduct.qty_available || 0))}`);
        ...
        for (let i = 0; i < extraSkusFound.length; i++) {
          ...
          try {
            ...
            state.products[missedSku] = {
              ...
              lastSynced: new Date().toISOString(),
              lastStock: Math.max(0, Math.floor(missedProduct.qty_available || 0)),
