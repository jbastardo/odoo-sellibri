...
async function buildFullPayload(
  ...
  const masterAttrs: sellibri.SellibriMasterAttributes = {
    ...
    stock_items_attributes: [{
      ...
      available: Math.max(0, Math.floor(odooProduct.available_quantity || odooProduct.qty_available || 0)),
    }],
  };
...
async function buildDiffPayload(
  ...
  const odooStock = Math.max(0, Math.floor(odooProduct.available_quantity || odooProduct.qty_available || 0));
    ...
    for (const p of products) {
      if (p.default_code) {
        odooDataMap.set(p.default_code, {
          qty: Math.max(0, Math.floor(p.available_quantity || p.qty_available || 0)),
